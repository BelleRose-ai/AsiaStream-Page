...
import { ShelfCategory } from '../data/shelves';
...
export const RecommendationShelf: React.FC<RecommendationShelfProps> = ({ category, index }) => {
  ...
  // Intersection Observer for smooth scroll-triggered CSS fade-in
  useEffect(() => {
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-4 pb-2 border-b border-white/5">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[11px] font-semibold tracking-wider text-[#06B6D4] uppercase">
                Category 0{index + 1}
              </span>
            ...
            <p className="text-xs sm:text-sm text-[#A0AEC0] mt-1 max-w-2xl">
              {category.description}
            </p>
          </div>

          {/* Desktop Shelf Scroll Controls (Functional, not card links) */}
          <div className="hidden sm:flex items-center gap-2 self-end">
            <button
              onClick={() => handleScroll('left')}
              disabled={!canScrollLeft}
              aria-label={`Scroll ${category.title} shelf left`}
              className={`p-2 rounded-full border border-white/10 transition-colors ${
                canScrollLeft
                  ? 'bg-[#161922] text-white hover:border-[#06B6D4] hover:text-[#06B6D4]'
            ...
            <button
              ...
              aria-label={`Scroll ${category.title} shelf right`}
              className={`p-2 rounded-full border border-white/10 transition-colors ${
                canScrollRight
                  ? 'bg-[#161922] text-white hover:border-[#06B6D4] hover:text-[#06B6D4]'
                  : 'bg-[#161922]/40 text-white/20 cursor-not-allowed border-transparent'
        ...
        <div
          ...
          tabIndex={0}
          role="region"
          aria-label={`List of ${category.title}`}
        >
